...
// My 2 prototypes for functions. I am still trying to wrap head around pointers.
int checker(FILE **image, FILE **output);
...
int main(int argc, char *argv[])
{
    ...
    // Call checker function and send the address for image and output.
    checker(&image, &output);
...
int checker(FILE **image, FILE **output)
{
    ...
    while (1)
    {
        // When the function is first called it will always fail this if check as the buffer will be empty.
        // The reason for putting the if check before fread is so when the function is recalled it doesn't rewrite the buffer then check.
        ...
        if (buffer[0] == 0xff && buffer[1] == 0xd8 && buffer[2] == 0xff && (buffer[3] & 0xf0) == 0xe0)
        {
            ...
            // After creating a file recall checker function.
            checker(*&image, *&output);
        }
